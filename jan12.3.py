...
def create_pdf_reportlab(club_info: dict, images: list, filename: str):
    """Create a PDF from the club information and images using ReportLab."""
    try:
        ...
        for img_path in images[:3]:  # Use up to 3 images
            elements.append(Image(img_path, width=4 * inch, height=3 * inch))
            elements.append(Spacer(1, 0.5 * inch))
...
async def main():
    ...
    create_pdf_reportlab(club_info, images, output_pdf)
